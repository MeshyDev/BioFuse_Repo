...
if %weaponarray% == 6 (if %weaponsix% == 1 (set weaponname=Bot Buster && set weapondmg=100 && echo It's a freakin' cannon.) else (set weaponarray=0 && set weaponname=Fists && set weapondmg=0 && echo It appears... it's not there. WHERES MY CANNON? && pause && exit /b))
if %weaponarray% == 7 (if %weaponseven% == 1 (set weaponname=Portable Death Laser && set weapondmg=750 && echo A military grade portable death laser.) else (set weaponarray=0 && set weaponname=Fists && set weapondmg=0 && echo Psh, yeah you wish! Like the government would ever let you have that && pause && exit /b))
exit /b