...
if %weaponarray% == 8 (if %weaponeight% == 1 (set weaponname=Gods Finger && set weapondmg=3500 && set weaponmod=Finger && echo A severed finger from God himself.) else (set weaponarray=0 && set weaponname=Fists && set weapondmg=0 && set weaponmod=Nothing && echo A weapon to eliminate and purge EVERYTHING. Unobtanium? && pause && echo No, it can be attained. && pause && exit /b))
exit /b